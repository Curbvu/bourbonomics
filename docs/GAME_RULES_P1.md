...
**v3.6 — effects are live.** The engine now resolves **30 of the 33 cards** (`implemented: true`). On-purchase cards fire the instant they land (Warehouse unlock, Aging Warehouse's +1 slot, Estate Bottling's free draft); the four **choice cards** — Brand Ambassador, Master Distiller, Climate-Controlled Warehouse, Bonded Warehouse — stash a pending choice that gates you to a single follow-up pick (barrel / tag / slot) before any other action. Automatic cards read ownership at the relevant trigger (sale, aging, end-game). **Free actions** ride dedicated moves: Warehouse store/retrieve, and Trade Lobby's once-per-round demand shift. **Three cards remain `implemented: false`** pending follow-up design and are marked **⏳** below: Field Office, Sales Office, Column Still.

The v3.6 catalog is **33 cards** distributed 9 small (cost 3–4) · 15 medium (cost 5–8) · 9 large (cost 9–15), grounded in real-world distillery capital expenditures.
