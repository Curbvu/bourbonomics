...
import { PLAYER_LOGOS } from "./playerLogos";
...
const DIFFICULTY_OPTIONS: { id: Difficulty; label: string; hint: string }[] = [
  { id: "easy", label: "Easy", hint: "Loosest heuristic" },
  { id: "normal", label: "Normal", hint: "Default heuristic" },
...
export default function MainMenu() {
  ...
  const canStart =
    totalSeats >= 2 && totalSeats <= 6 && playerName.trim().length > 0;

  const setBotDifficulty = (idx: number, difficulty: Difficulty) => {
  ...
  const addBot = () => {
    ...
    const next =
      BOT_NAME_POOL.find((n) => !used.has(n)) ?? `Bot ${bots.length + 1}`;
  ...
  const start = () => {
    ...
    newGame({
      ...
      human: { name: playerName.trim() || "You", logoId: PLAYER_LOGOS[0]!.id },
      bots: bots.map((b, i) => ({
        name: b.name.trim() || "Bot",
        difficulty: b.difficulty,
        logoId: PLAYER_LOGOS[(i + 1) % PLAYER_LOGOS.length]!.id,
      })),
    });
  };
