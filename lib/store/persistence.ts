  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && parsed.version === 8) {
        ...
        // Old / unknown schema — drop it so the page boots into a fresh
        // game. The Distillery rewrite changed too many shapes (player
        // fields, market.distilleryDeck, removed freeActions field, new
        // phase enum value) to make a soft migration worth the risk.
